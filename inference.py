...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges.extend([(PAC,OBS1), (PAC, OBS0), (GHOST0, OBS0), (GHOST1, OBS1)])
    ...
    variableDomainsDict[OBS0].extend(range(0,X_RANGE+Y_RANGE-1+MAX_NOISE))
    variableDomainsDict[OBS1].extend(range(0,X_RANGE+Y_RANGE-1+MAX_NOISE))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        allprobs = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for evar in eliminationOrder:
            joined = joinFactorsByVariable(allprobs, evar)
            allprobs = joined[0]
            fac = joined[1]
            if len(fac.unconditionedVariables()) > 1:
                allprobs.append(eliminate(fac, evar))
        return normalize(joinFactors(allprobs))
    ...
    def normalize(self):
        ...
        if s != 0:
            for key in self.keys():
                self.update({key:self.get(key)/s})
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            return 0
        return busters.getObservationProbability(noisyDistance= noisyDistance, trueDistance= manhattanDistance(pacmanPosition, ghostPosition))
        "*** END YOUR CODE HERE ***"
